...
public class Main {
    ...
    public static void fatDummyExample() {
        ...
        sc.parallelize( DataLake.Datasets.pool( "gs:2021/01/201025410/137946" ).asLines( 1000000 ) )
                .map( line -> DataLake.Model.buildInstance( line , "," ) )
                .flatMap( item -> Arrays.asList( item.getStrValue().split( "\\d{4}-\\d{2}-\\d{2}" ) ).stream()
                .map( c -> DataLake.Model.getReply( item , String.valueOf( c ) ) ).collect( Collectors.toList() ).iterator() )
                ...
                .foreach( DataLake.Datasets :: feed );
    ...
    public static void dummyReducedByKeyFluentExample() {
        ...
        sc
                .parallelize( DataLake.Datasets.pool( "gs:2021/01/201025410/137946" ).asModels( 1000 ) )
                .mapToPair( item -> new Tuple2<>( String.valueOf( item.getChar() ) ,
                                                  new Tuple2<>( item.getIntValue() , item.getDoubleValue() ) ) )
                .reduceByKey(
                        ( a , b ) -> new Tuple2<>( a._1 + b._1 , a._2 + b._2 ) )
    ...
    public static void dummyReducedByKeyExample() {
        final SparkConf conf = SparkConfigBuilder.getInstance( "Dummy" , MasterType.LOCAL , Cores.ALL );
        JavaSparkContext sc = new JavaSparkContext( conf );
        final JavaRDD<DataLake.Model> rdd = sc.parallelize(
                DataLake.Datasets.pool( "gs:2021/01/201025410/137946" ).asModels( 1000 ) );
    ...
    public static void dummyWithTupleExample() {
        ...
        final JavaRDD<DataLake.Model> rdd = sc.parallelize( DataLake.Datasets.pool( "a" ).
                asModels( 1000 ) );
        final JavaRDD<Tuple2<DataLake.Model , Double>> rddTuple = rdd.map( item -> new Tuple2<>( item ,
                                                                                                 item.getDoubleValue() ) );
